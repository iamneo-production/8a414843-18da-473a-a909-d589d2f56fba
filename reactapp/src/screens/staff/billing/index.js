...
const getBilling =async() =>{
    await get(`${EndPoints.getBilling}?appointmentStatus=prescribed`).then((response)=>{
...
const colDef = [
    ...
    },
    {
        ...
        textAlignment: "center",
        render: (data) => (
        <Button size='xs' onClick ={()=>{setInvoicemodal(true);setBillGenerating(data)}}> Pay</Button>
        ),
    }
];
...
//   }, [searchValue, records]);
    ...
    return (
        ...
        <CustomTable coloumnDef={colDef} records={billingData} />
    </Box>

    {invoicemodal && <BillingModal open={invoicemodal} close={()=>{setInvoicemodal(false)}} records={billGenerating}/>}
